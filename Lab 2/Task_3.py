...
def heuristic(positions):
    return max([closest_distances(pos) for pos in positions])
...
def A_star(positions):
    ...
    heappush(heap, (heuristic(positions), positions, ''))
    ...
    while heap:
        ...
        for direction in 'UDLR':
            ...
            h = heuristic(new_commando) + len(path)
